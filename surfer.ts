...
function RemakeData (d : OldSearchDataType) {
  let n : CateSearchDataType = {
    adult: d.live.adult,
...
let els : {[key : string] : HTMLElement};

let disReverse = false;

type adultType = ('baby' | 'adult' | 'only');
let adult : adultType = 'baby';
...
let intag = '';
let extag = '';
let intag_list : string[] = [];
let extag_list : string[] = [];
...
let selected_cateId : string[] = []; // 선택된 카테고리 id
...
let dis1_filter : CategoriesDataType[] = []; // 보고싶은 거만 보기
let live_list : CateSearchDataType[] = []; // 생방송 자료 원본 박물관
let live_list_content : CateSearchDataType[] = []; // 콘텐트에서 처리한 자료 원본 박물관
let live_list_fin : CateSearchDataType[] = []; // 최종 보여줄 라이브
...
document.addEventListener('DOMContentLoaded', async() => {
  ...
  if (getCookie('isSaved') === 'true') {
    ...
    if (isValueElement(els.intag)) els.intag.value = getCookie('intag');
    if (isValueElement(els.extag)) els.extag.value = getCookie('extag');
  ...
  els.searchActionButton?.addEventListener('click', () => {
    last_time = do_time;
    do_time = now();
    if ((Number(do_time) - Number(last_time)) < 100) return;
    inputScan();
  ...
  // background로부터 수신 받기
  chrome.runtime.onMessage.addListener((message: MessageRequest, sender: chrome.runtime.MessageSender, sendResponse: (response?: any) => void) => {
...
function preventPageScroll(e: WheelEvent) {
  ...
  const isAtBottom = (element: HTMLElement) => element.scrollHeight - element.clientHeight - element.scrollTop <= 1; // 1px 오차 허용
...
// ================ 쿠키 함수 ================
// 쿠키 저장 함수
function setCookie(name: string, value: string, expireDays: number = 7): void {
  const date = now();
...
// 쿠키 가져오기 함수
function getCookie <T = string> (name: string): T {
  ...
  for(let i = 0; i < cookieArray.length; i++) {
    ...
    if (cookie.indexOf(cookieName) === 0) {
      return cookie.substring(cookieName.length, cookie.length) as T;
    }
  }
  return "" as T;
}
...
// ts 타입가드 런타임 함수
function isValueElement(element: HTMLElement): element is HTMLInputElement {
...
async function searchKeyword(keyword : string) {
  ...
  let new_rows : CateSearchDataType[] = rows.map((row)=>RemakeData(row));
  return new_rows;
}
...
const regexhm1 = /^(\d+):(\d+)?$/;
// 시간 입력값을 ms로 바꾸는 함수
function inputtime(time: string, temp: number = 0): number {
...
// 필터 적용해서 최종 렌더하는 함수
async function RenderStreams(n : number = 120, offset : number = 0) {
  ...
  for (const live of live_list_filter_sliced) {
    ...
    img.onerror = async function() {
      ...
      if (load_n > 20) this.onerror = null;
      await sleep(2000);
      img.src = live.defaultThumbnailImageUrl || live.liveImageUrl.replace("{type}", "480");
...
// surfer.ts에서 background.js로 메시지 보내기
interface MessagePayload {
  action: string;
  data?: any;
}

function sendToBackground(message: MessagePayload): void {
...
function live_list_filterMaker (q : string = "") {
  let live_list_up : CateSearchDataType[] = [];
  ...
  dis1_filter = [];
  let cate_fixed_live_list : CateSearchDataType[] = [];
  ...
  // 태그 검색 필터
  let tag_fixed_live_list : CateSearchDataType[] = [];
  if (tagSearchSw) {
    for (const l of cate_fixed_live_list) {
      let tagIn_e : CateSearchDataType | null = null;
  ...
  // 19 검색 필터
  let adult_fixed_live_list : CateSearchDataType[] = [];
  ...
  // 방송시간 필터
  let time_fixed_live_list : CateSearchDataType[] = [];
  ...
  let view_fixed_live_list : CateSearchDataType[] = [];
  ...
  let sort_fixed_live_list : CateSearchDataType[] = [];
  if (disReverse) {
...
async function miniSearch(query : string, tag : boolean = false) {
...
function DomUpdate () {
  els = {
    searchActionButton: document.getElementById('searchActionButton') as HTMLElement,
    viewBtn: document.getElementById('viewBtn') as HTMLElement,

    searchInput: document.getElementById('searchInput') as HTMLElement,
    intag: document.getElementById('intag') as HTMLElement,
    extag: document.getElementById('extag') as HTMLElement,
    mintime: document.getElementById('mintime') as HTMLElement,
    maxtime: document.getElementById('maxtime') as HTMLElement,
    minview: document.getElementById('minview') as HTMLElement,
    maxview: document.getElementById('maxview') as HTMLElement,

    adult: document.getElementById('adult') as HTMLElement,
    tagSearchSw: document.getElementById('tagSearchSw') as HTMLElement,

    ProgressBar: document.getElementById('ProgressBar') as HTMLElement,
    Surfer: document.getElementById('Surfer') as HTMLElement,
    CateCheckboxContainer: document.getElementById('CateCheckboxContainer') as HTMLElement,
  }
  return els;
}

// 태그 입력값 array로 변환
function filtertolistMaker2 (i : string) {
  let r : string[] = [];
  const arr = i.split(',');
...
type CateTypeTypeK = '전체' | '게임' | '기타' | '엔터테이먼트' | '스포츠'
function RenderCateCheckbox (dis1 : CategoriesDataType[]) {
  ...
  const CatetypeK : CateTypeTypeK[] = ['전체', '게임', '기타', '엔터테이먼트', '스포츠'];
  const Catetype = [
    {
      k: CatetypeK[0],
      view: viewdata.전체,
  ...
  for (const c of Catetype) {
    ...
    btn.addEventListener('click', function() {
      if (c.id === 'ALL') {
        ...
        this.style.backgroundColor = color.cate.none;
        const type_ex_a : CategoryTypeType[] = ['GAME', 'ETC', 'ENTERTAINMENT', 'SPORTS'];
